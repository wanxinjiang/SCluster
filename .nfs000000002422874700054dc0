...
def SClustering(adata,c_nums=None,sc3=False,soup=False,seurat=False,RaceID3=False,
    ...
    SC=Methods(adata,c_nums,results,_temp)
...
def run_simlr(simlr,_simlrout_csv):
    if simlr:
...
def run_RaceID3(RaceID3,_RaceID3out_csv):
    if RaceID3:
...
def run_seurat(seurat,_seuratout_csv):
    if seurat:
        print('Seurat done.')
...
class Methods(object):
    def __init__(self,adata,c_nums,results,temp):
        ...
        self._matrix_raw=temp+'matrix_raw.csv'
        self._matrix=temp+'matrix.csv'
        ...
        self.matrix.to_csv(self._matrix)
        self.matrix_raw.to_csv(self._matrix_raw)
    ...
    def SC_scanpy(self):
        ...
        if int(self.c_nums)<=3:
            ...
            for i in range(1,151):
                sc.tl.leiden(self.scanpyout,resolution=(i)*0.01)
                cluster=self.scanpyout.obs["leiden"] 
                if abs(len(np.unique(cluster))-int(self.c_nums))==1:
                    scanpy_cluster_1=self.scanpyout.obs["leiden"] 
                if len(np.unique(cluster))==int(self.c_nums):
                            break     
                if i==151:
                    cluster=scanpy_cluster_1 
