seurat_SCluster <- function(c_nums=NULL,inputmatrix,save.results=FALSE){
    ...
    for (i in 1:150){
    seuratout <- FindClusters(seuratout,resolution=i*0.01,verbose=FALSE)

    if(abs(as.integer(length(unique(seuratout@active.ident)))-c_nums)==1){
        seuratout_cluster_1=seuratout
    }
