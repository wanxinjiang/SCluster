...
sincera_SCluster<- function(c_nums=NULL,inputmatrix,save.results=FALSE){
    ...
    #filter cell
    # sinceraout <- filterLowQualityCells(sinceraout, min.expression=1, min.genes=1000, do.plot = T)
    ...
    obj <- cluster.geneSelection(sinceraout, method="specificity", pergroup=TRUE)
    sincera_sc <- setGenesForClustering(sinceraout,value=getGenesForClustering(obj))
...
RaceID3_SCluster <- function(c_nums=NULL,inputmatrix,save.results=FALSE){
    ...
    RaceID3out <- clustexp(RaceID3out,cln=c_nums)
